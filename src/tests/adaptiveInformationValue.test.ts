...
describe("adaptive role-family information value", () => {
  ...
  it("is deterministic for identical evidence", () => {
    ...
    expect(questionInformationValue(discoveryQuestionById["r-rope-give"], plausible, {})).toEqual(questionInformationValue(discoveryQuestionById["r-rope-give"], plausible, {}));
  });

  it("does not ask arbitrary refinements after twenty unknown broad answers", () => {
    const answers = emptyAnswers();
    Object.assign(answers.discovery, Object.fromEntries(discoveryQuestions.filter((question) => question.phase === "broad").map((question) => [question.id, "unknown"])));
    const traitScores = calculateTraitScores(answers.discovery);
    const plausible = matchRoles(traitScores, answers.discovery).filter((result) => result.relevantAnswers > 0 && result.rawScore >= 0.38);
  ...
  it("preserves evidence-backed refinement and the six-question adaptive cap", () => {
    const answers = emptyAnswers();
    Object.assign(answers.discovery, Object.fromEntries(discoveryQuestions.filter((question) => question.phase === "broad").map((question) => [question.id, question.id === "d-rope" ? "strong" : "unknown"])));
    const asked: string[] = [];
    while (true) {

...
export function RoleProfileBuilderLauncher({ roleResults, onRoleSetChange }: { roleResults: RoleResult[]; onRoleSetChange: (roles: EditableRoleProfileEntry[]) => void }) {
    ...
    <details className="profile-builder-disclosure" open={isOpen || isClosing} data-closing={isClosing || undefined}>
      ...
      <div className="profile-builder-content">
        <div className="profile-builder-content-inner">
          ...
          <Suspense fallback={<p role="status">Loading role suggestions…</p>}><LazyRoleProfileBuilder roleResults={roleResults} embedded onRoleSetChange={onRoleSetChange} /></Suspense>
        </div>
      </div>
    </details>
  </section>

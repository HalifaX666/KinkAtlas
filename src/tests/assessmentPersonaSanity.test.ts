...
const substantivePersonaIds = new Set(assessmentPersonas.slice(0, 12).map((persona) => persona.id).concat('multiple-hard-limits-pattern'))
...
describe('assessment persona fixture validation', () => {
  it('contains 15 completed, uniquely identified personas', () => {
    expect(assessmentPersonas).toHaveLength(15)
  ...
  it('fails clearly for duplicate IDs, unknown questions, invalid options, and malformed sections', () => {
    expect(() => createAnswerRecord('discovery', [['d-rope', 'strong'], ['d-rope', 'no']])).toThrow('discovery: duplicate question ID d-rope')
...
describe.each(assessmentPersonas)('Persona: $name', (persona) => {
  it('Invariant: evaluates the complete Results pipeline locally with valid finite structures', () => {
    ...
    expect(roleResultIds.every((id) => roleIds.has(id)), `${persona.id}: Role Discovery contains an unknown role ID`).toBe(true)
    if (substantivePersonaIds.has(persona.id)) expect(evaluation.roleResults.some((result) => result.relevantAnswers > 0), `${persona.id}: expected at least one evidenced role`).toBe(true)
    ...
    for (let index = 1; index < evaluation.roleResults.length; index += 1) {
      ...
      const correctlyOrdered = previous.rankedScore > current.rankedScore
        || (previous.rankedScore === current.rankedScore && previous.rawScore > current.rawScore)
        || (previous.rankedScore === current.rankedScore && previous.rawScore === current.rawScore && previous.relevantAnswers > current.relevantAnswers)
        || (previous.rankedScore === current.rankedScore && previous.rawScore === current.rawScore && previous.relevantAnswers === current.relevantAnswers && previous.role.id.localeCompare(current.role.id) <= 0)
    ...
    expect(suggestedIds.every((id) => publicRoleIds.has(id)), `${persona.id}: Suggested Role Set contains an unknown public role`).toBe(true)
...
describe('role-set and export constitution', () => {
  it('keeps Suggested Role Set separate from first-five Role Discovery assumptions', () => {
    const hasIndependentSuggestion = assessmentPersonas.some((persona) => {
      const evaluation = evaluateAssessmentPersona(persona)
      const discoveryFirstFive = evaluation.roleResults.filter((result) => result.alignment !== 'insufficient').slice(0, 5).map((result) => result.role.name)

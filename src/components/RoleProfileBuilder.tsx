...
export function RoleProfileBuilder({ roleResults, embedded = false, onRoleSetChange }: { roleResults: RoleResult[]; embedded?: boolean; onRoleSetChange?: (roles: EditableRoleProfileEntry[]) => void }) {
  ...
  const content = <>
    ...
    <div className="profile-builder-grid">
      <div className="profile-role-editor">
        <header><div><h3>Your role set</h3><p>Make it yours: reorder, replace, remove, or add roles. Shareable cards always use this current set.</p><p>{selectedRoles.length} of 5 roles in your set; fewer or none is valid. The first role is primary within this role set.</p></div></header>
      ...
      <div className="profile-catalog-search">
        ...
        {relatedRoles.length ? <ul className="profile-search-results related-role-results">{relatedRoles.map((role) => <li key={role.roleId}><span><strong>{role.label}</strong><small>Related · {role.reason}</small></span><button type="button" className="quiet-button" disabled={selectedIds.has(role.roleId)} aria-label={`Add related role ${role.label}`} onClick={() => addRole(role.roleId, role.label, roleLibraryRoleById.get(role.roleId)?.definition)}><Plus size={17} />{selectedIds.has(role.roleId) ? 'Selected' : 'Add'}</button></li>)}</ul> : <p>No relationship-reviewed nearby roles are available for the current recommendations.</p>}
        </section>

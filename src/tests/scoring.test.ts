...
describe("differentiated discovery scoring", () => {
  ...
  it("keeps dominance and submission independent and can support Switch", () => {
    ...
    expect(traits.dominance?.value).toBeGreaterThan(0.7);
    expect(traits.submission?.value).toBeGreaterThan(0.7);
  ...
  it("keeps unrelated strong interests from cancelling one another", () => {
    ...
    expect(traits.painGiving?.value).toBeGreaterThan(0.8);
    expect(traits.caregiving?.value).toBeGreaterThan(0.8);
  });
...
describe("role-discrimination adaptive questioning", () => {
  const broadAnswers = (overrides: Record<string, string> = {}) => Object.fromEntries(discoveryQuestions.filter((question) => question.phase === "broad").map((question) => [question.id, overrides[question.id] ?? "no"]));

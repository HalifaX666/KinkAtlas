...
function completeDiscovery(seed: PersonaSeed): AssessmentAnswers['discovery'] {
  const discovery = createAnswerRecord('discovery', broadQuestions.map((question) => [question.id, seed.discovery?.[question.id] ?? seed.discoveryDefault ?? 'no']))
  while (Object.keys(discovery).length < 26) {
    const next = selectNextDiscoveryQuestion({ discovery, readiness: {}, boundaries: {}, negotiation: {} }, calculateTraitScores(discovery))
    if (!next) break
...
function createPersona(seed: PersonaSeed): AssessmentPersona {
  return {
    ...
    answers: {
      ...
      readiness: createAnswerRecord('readiness', readinessQuestions.map((question) => [question.id, seed.readinessDefault ?? 'a'])),
      ...
      negotiation: createAnswerRecord('negotiation', negotiationQuestions.map((question) => [question.id, seed.negotiationDefault ?? 'p4'])),
    },
  }
}
